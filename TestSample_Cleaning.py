...
rtdir = os.path.join(os.path.dirname(__file__),"DATA","LED +APs 240926")
chans = ["ChanA","ChanB"]

print(f"Cleaning subfolders in: {rtdir}...")
...
for chan in chans:
    ...
    print(os.path.join(rtdir,sub_1,'DATA',chan,'stats.txt'))
    if os.path.isfile(os.path.join(rtdir,sub_1,'DATA',chan,'stats.txt')):
        print(f"Removing stats.txt in {sub_1}")
        os.remove(os.path.join(rtdir,sub_1,'DATA',chan,'stats.txt'))
    if os.path.isfile(os.path.join(rtdir,sub_2,'DATA',chan,'stats.txt')):
        os.remove(os.path.join(rtdir,sub_2,'DATA',chan,'stats.txt'))
    if os.path.isfile(os.path.join(rtdir,sub_3,"DATA",chan,"stats.txt")):
        os.remove(os.path.join(rtdir,sub_3,"DATA",chan,"stats.txt"))
